    ...
    let filteredData = data?.items.item?.filter(item => {
        if (selectedTag == null || selectedTag == "") {
            ...
            if (selectedTag.includes('계곡옆')) {
                valid = valid && item.lctCl.includes('계곡') || valid && item.posblFcltyCl.includes('계곡')
            }
            if (selectedTag.includes('차대기 편한')) {
                valid = valid && item.induty.includes('자동차야영장') ||(item.intro.includes('넓은 주차장') || item.featureNm.includes('넓은 주차장') || item.lineIntro.includes('넓은 주차장')) 
            }
            if (selectedTag.includes('그늘이 많은')) {
                valid = valid && item.lctCl.includes('숲') || (item.intro.includes('그늘') || item.featureNm.includes('그늘') || item.lineIntro.includes('그늘'))
            }
            if (selectedTag.includes('문화유적')) {
                valid = valid && item.clturEventAt.includes('Y')
            }
            if (selectedTag.includes('둘레길') || selectedTag.includes('자전거 타기 좋은')) {
                valid = valid && (item.posblFcltyCl.includes('산책로') ||
                    item.themaEnvrnCl.includes('걷기길')) || (item.intro.includes('자전거') || item.featureNm.includes('자전거') || item.lineIntro.includes('자전거'))
            }
            
            //설명
            if (selectedTag.includes('깨끗한')) {
                valid = valid && (item.intro.includes('깨끗') || item.featureNm.includes('깨끗') || item.lineIntro.includes('깨끗'))
            }
            ...
            if (selectedTag.includes('물맑은')) {
                valid = valid && (item.intro.includes('맑은') || item.featureNm.includes('맑은') || item.lineIntro.includes('맑은'))
            }
            if (selectedTag.includes('여유있는')) {
                valid = valid && (item.intro.includes('여유') || item.featureNm.includes('여유') || item.lineIntro.includes('여유'))
            }
            if (selectedTag.includes('친절한')) {
                valid = valid && (item.intro.includes('친절') || item.featureNm.includes('친절') || item.lineIntro.includes('친절'))
            }
