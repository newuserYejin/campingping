...
const BestCamp = () => {
  ...
  const BestCampList = data?.filter((d) => {
    const list = d.contentId === '100069' ||  d.contentId === '100073' || d.contentId === '100033' || d.contentId === '100106';
    return list;
  });
  ...
  return (
    <section className='mainBestCamp'>
      <Slider {...settings} ref={slickRef}>
        {BestCampList && BestCampList.map((item) => 
          <div key={item.contentId}>
            <article className='mainBestCampItem'>
              <div className="textBox">
                <div>
                  {item.contentId === '100069' && 
                    <h3>아이들과 가기 좋은</h3>
                  }
                  {(item.contentId === '100073' || item.contentId === '100033') && 
                    <h3>반려동물과 함께하는</h3>
                  }
                  {item.contentId === '100106' && 
                    <h3>차박하기 좋은</h3>
                  }
                  <h4>{item.facltNm}</h4>
                  <p>{item.intro}</p>
              ...
              <div className="imgBox">
                <img src={item.firstImageUrl} alt={`${item.facltNm} 이미지`} />
              </div>
            </article>
          </div>
          )}
