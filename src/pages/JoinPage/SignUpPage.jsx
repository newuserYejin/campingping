...
import { validateEmail, validatePassword, validateContact, validateNickname } from "../../utils/common";
...
const SignUpPage = () => {
  ...
  const handleChange = (event) => {
    ...
    if (name === "email" && !validateEmail(value)) {
      ...
      error = "비밀번호는 8자 이상이며, 문자, 숫자, 특수문자를 포함해야 합니다.";
  ...
  const handleSubmit = (event) => {
    ...
    if (!validatePassword(formData.password))
      newErrors.password = "비밀번호는 8자 이상이며, 문자, 숫자, 특수문자를 포함해야 합니다.";
    if (!validateContact(formData.contact)) newErrors.contact = "유효한 연락처 번호를 입력해주세요.";
